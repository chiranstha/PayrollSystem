...
namespace Suktas.Payroll.Install
{
    [AbpAllowAnonymous]
    [DisableAuditing]
    public class InstallAppService : PayrollAppServiceBase, IInstallAppService
    {
        private readonly AbpZeroDbMigrator<PayrollDbContext> _migrator;
        ...
        {
            var connectionString = _appConfiguration[$"ConnectionStrings:{PayrollConsts.ConnectionStringName}"];
        ...
        {
            EditAppSettingsjson($"ConnectionStrings:{PayrollConsts.ConnectionStringName}", constring);
        }
